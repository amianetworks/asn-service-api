...
type TSCollOptions struct {
	...
	TSFileDBOpts TSFileDBOptions // Required when creating file time-series collection, otherwise ignored.
}

type TSMongoOptions struct {
	TimeField   string   // Required when creating mongo time-series collection, otherwise ignored.
	Indexes     []string // key = "column1;column2;..."
	MetaField   string   // Optional, Set only when creating mongo time-series collection, otherwise ignored.
	Granularity string   // Optional, Set only when creating mongo time-series collection, otherwise ignored.
	Expired     string   // Optional, Set only when creating mongo time-series collection, otherwise ignored.
}
type TSFileDBOptions struct {
	// Optional, when creating file time-series collection, otherwise ignored.
...
type TSDBHandler interface {
	...
	//	for influxdb, indexes is nil
	//	for prometheus, indexes is nil
	//
...
type TSPoint interface {
	...
	// And now, do not limit the amount of Tags
	AddTag(tagName, tagValue string) error

	// DeleteTag deletes a tag from tags
	DeleteTag(tagName string) error

	// AddTags adds some tags to point, will overwrite the value if set the same key repeatedly
	// And now, do not limit the amount of Tags
	...
	// And now, do not limit the amount of Fields
	...
	// SetRPName sets rp for point to insert.
...
type TSQuery interface {
	// AddTag adds a tag to query, will overwrite the value if set the same key repeatedly
	// And now, do not limit the amount of Tags
	...
	// SetStep sets query step, will change input to time.Duration.
...
// TSSamples is a sample stream, amount of SamplePoint with same tags group a sample stream
type TSSamples struct {
	Tags   map[string]string
	Values []TSSamplePoint
}

// TSMatrix key: tags group marshalled string, different tags group according to different Samples
type TSMatrix map[string]*TSSamples

// TSQueryResult is return value after execution one time query.
// key: field name, values with same field-name stored in same one Matrix
...
type TSCollection interface {
	...
	// FindDataPoints finds data points in collection c.
	// This function do not care how many data points returned in the process, maybe 0, 1, or more.
