...
type IAM interface {
	...
	Login(sysType, username string, password []byte, userClaims string) (needMFAVerify bool, qrImg, secret, accessToken, refreshToken string, err error) // Login allows the user system to  login.
	PasswordVerify(username string, password []byte) (err error)                                                                                         // PasswordVerify allows the user system to check password of user.
	MFAVerify(sysType, username string, mfaCode int32, mfaType string) (err error)                                                                       // MFAVerify allows the user system to perform Multi-Factor Authentication.
	MFALoginVerify(sysType, username string, mfaCode int32, mfaType, userClaims string) (accessToken, refreshToken string, err error)                    // MFALoginVerify allows the user system to perform Multi-Factor Authentication while login.
	AuthenticatorBindConfirm(sysType, username string, mfaCode int32, secret string) (err error)                                                         // AuthenticatorBindConfirm allows the user system to perform Multi-Factor Authentication.
	AuthenticatorBind(sysType, username string) (qrImg, secret string, err error)                                                                        // AuthenticatorBind allows the user to bind when Authenticator is needed.
